...
public class MakeUtils {
    ...
    private static final String XML_DIMEN_TEMPLETE = "<dimen name=\"dp%1$s\">%2$.2fdp</dimen>\r\n";

    private static final String XML_DIMEN_TEMPLETE_SP = "<dimen name=\"sp%1$s\">%2$.2fsp</dimen>\r\n";
    ...
    private static String makeAllDimens(DimenTypes type, int designWidth) {
        ...
        try {
            ...
            for (int i = 0; i <= designWidth; i++) {
                ...
                dpValue = px2dip((float) i+0.5f,type.getSwWidthDp(),designWidth);
                temp = String.format(XML_DIMEN_TEMPLETE, i+"_5", dpValue);
                sb.append(temp);
            ...
            for (int i = 0; i <= 35; i++) {
                ...
                dpValue = px2dip((float) i+0.5f,type.getSwWidthDp(),designWidth);
                temp = String.format(XML_DIMEN_TEMPLETE_SP, i+"_5", dpValue);
                sb.append(temp);
    ...
    public static void makeAll(int designWidth, DimenTypes type, String buildDir) {
        try {
            ...
            if (type.getSwWidthDp() > 0) {
                //适配Android 3.2+
                folderName = "values-w" + type.getSwWidthDp() + "dp";
            }else {
                return;
            }
